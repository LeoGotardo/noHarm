...
const G_PATH = 'M44.5 20H24v8.5h11.8C34.7 33.9 30.1 37 24 37c-7.2 0-13-5.8-13-13s5.8-13 13-13c3.1 0 5.9 1.1 8.1 2.9l6.4-6.4C34.6 5.1 29.6 3 24 3 12.4 3 3 12.4 3 24s9.4 21 21 21c10.5 0 20-7.6 20-21 0-1.4-.1-2.7-.5-4z'
...
export function GoogleButton({
  ...
  const bg     = isSignup ? (disabled ? 'var(--surface)' : 'var(--primary)') : 'var(--surface)'
  const border  = isSignup && !disabled ? 'none' : '1.5px solid var(--border)'
  const color   = isSignup ? (disabled ? 'var(--ink-3)' : 'var(--on-primary)') : 'var(--ink)'
  const spinnerBorder = isSignup ? 'rgba(255,255,255,0.3)' : 'var(--border)'
  const spinnerTop    = isSignup ? '#fff' : 'var(--primary)'

  const defaultLabel       = isSignup ? 'Sign up with Google' : 'Continue with Google'
  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: 15.5, fontWeight: 600, color,
