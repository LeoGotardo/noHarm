...
import { Banner, Toast, BottomSheet, TabBar, Screen } from '@components'
...
const FRIENDSHIP = { ACCEPTED: 5 }
const MSG        = { UNREAD: 7 }

// ── Theme defaults ────────────────────────────────────────────────────────────
const TWEAK_DEFAULTS = { direction: 'sage', mode: 'light', motion: true, accentName: 'warm' }
...
function StartStreakSheet({ open, date, loading, onChangeDate, onConfirm, onClose }) {
  return (
    <BottomSheet open={open} onClose={onClose}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--ink)', fontFamily: 'var(--font-display)' }}>When did you start?</div>
        <div style={{ fontSize: 14.5, color: 'var(--ink-2)', marginTop: 10, lineHeight: 1.55, padding: '0 4px' }}>
          Pick the date you began your recovery. If it was today, just leave it as is.
        </div>
      </div>
  ...
  return (
    <BottomSheet open={open} onClose={onClose}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 24 }}>
        <Btn kind="primary" full onClick={onConfirm} disabled={loading}>Reset & start fresh</Btn>
        <Btn kind="ghost" full onClick={onClose}>Not now</Btn>
      </div>
    </BottomSheet>
  )
}
...
export default function App() {
  ...
  const {
    streak, record, days,
    checkedIn, needsCheckin, missedDays, lastCheckinDate,
    checkIn, relapse: doRelapse, performCheckin, startFrom,
  ...
  const friends     = friendshipData.friendships?.filter(f => f.status === FRIENDSHIP.ACCEPTED) ?? []
  ...
  const chatUnread = useMemo(() =>
    chatList.reduce((n, c) => {
      ...
      return n + msgs.filter(m => m.status === MSG.UNREAD && m.sender !== me?.id).length
    }, 0)
  , [chatList, me])
  ...
  const personalRecord = record
    ? Math.floor((new Date(record.end ?? Date.now()) - new Date(record.start)) / 86_400_000)
    : 0
  ...
  const { requestPermission: enableNotifications, granted: notifGranted } = useNotifications(phase === 'app' ? me?.id : null, notifPrefs)
  const reminderEnabled = phase === 'app' && notifGranted && notifPrefs.master && notifPrefs.checkinReminder
  ...
  const openProfile = (userId) => {
    const isFriend = friends.some(f => f.sender === userId || f.reciver === userId)
    const isSent   = reqSent.some(r => r.reciver === userId)
    const isRecv   = reqReceived.some(r => r.sender === userId)
    const relation = isFriend ? 'friend' : isSent ? 'pending_out' : isRecv ? 'pending_in' : 'none'
  ...
  const messagePerson = (userId) => {
    const chat = chatList.find(x => x.sender === userId || x.reciver === userId)
    resetTo('chat')
    // Small delay so the tab transition renders before the stack push
    setTimeout(() => push('chatThread', { chat: chat ?? { id: null, sender: me?.id, reciver: userId, messages: { messages: [], total: 0 } } }), 30)
  }
  ...
  if (phase === 'splash') {
    ...
    if (top) {
      ...
      switch (top.screen) {
        case 'history':
          body = <StreakHistory onBack={pop} currentDays={days} currentStart={startLabel} empty={days === 0} />
          break
        case 'requests':
          body = <FriendRequests onBack={pop} received={reqReceived} sent={reqSent} meId={me?.id}
            ...
            onDecline={async () => { await refetchFriends() }}
            onCancel={async () => { await refetchFriends() }}
            onOpenProfile={openProfile} />
          break
        case 'search':
          body = <FriendSearch onBack={pop} pool={[]} onOpenProfile={openProfile}
            onSendRequest={async () => { showToast('Request sent') }} />
          break
        case 'publicProfile':
          body = <PublicProfile onBack={pop} userId={top.props.userId} relation={top.props.relation}
            ...
            onBlock={async () => { await refetchFriends(); showToast('User blocked') }} />
          break
        case 'chatThread':
          body = <ChatThread onBack={pop} chat={top.props.chat} meId={me?.id} onOpenProfile={openProfile} />
          break
        case 'badgeDetail':
          body = <BadgeDetail onBack={pop} badge={top.props.badge} currentDays={days} justUnlocked={top.props.justUnlocked} />
          break
        case 'edit':
          body = <EditProfile me={me} onBack={pop} onSave={() => { pop(); refetchMe(); showToast('Profile updated') }} />
          break
        case 'settings':
          body = <Settings onBack={pop} mode={mode}
            onToggleMode={() => setTweak('mode', mode === 'dark' ? 'light' : 'dark')}
            onLogout={() => { setStack([]); setPhase('splash') }}
            onDeleted={() => { setStack([]); setPhase('deleted') }}
      ...
      switch (tab) {
        case 'home':
          body = <Dashboard
            me={me} days={days} hasStreak={!!streak} checkedIn={checkedIn}
            milestone={milestone} startLabel={startLabel}
            personalRecord={personalRecord} totalStreaks={0}
            nextBadgeName={nextBadge?.name ?? ''} pulseKey={pulseKey}
            onCheckIn={onCheckIn}
            onRelapse={() => setRelapseOpen(true)}
            onOpenHistory={() => push('history')}
            onProfile={() => resetTo('profile')}
            onStartStreak={() => { setStartDate(todayISO()); setStartOpen(true) }}
          />
          break
        case 'friends':
          body = <FriendsScreen friends={friends} meId={me?.id} requestCount={reqReceived.length}
            ...
            onOpenProfile={openProfile} onMessage={messagePerson} />
          break
        case 'chat':
          body = <ChatList chats={chatList} meId={me?.id} onOpen={openChat} onOpenProfile={openProfile} />
          break
        case 'badges':
          body = <BadgesScreen badges={liveBadges} currentDays={days}
            onOpen={id => push('badgeDetail', { badge: liveBadges.find(b => b.id === id) })} />
          break
        case 'profile':
  ...
  return (
    <div className="nh-root" data-dir={dir} data-mode={mode} data-reduce-motion={motion ? 'no' : 'yes'}>
      <div id="nh-screen" style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>

        {/* Animated screen container — key change triggers nhScreenIn */}
        <div
          key={phase + tab + (stack.length ? stack[stack.length - 1].screen : '')}
        ...
        {showTabs && (
          <TabBar active={tab} onChange={resetTo}
      ...
      <TweaksPanel>
        ...
        <TweakRadio label="Theme" value={dir} options={['sage', 'dawn']} onChange={v => setTweak('direction', v)} />
        <TweakRadio label="Appearance" value={mode} options={['light', 'dark']} onChange={v => setTweak('mode', v)} />
        <TweakSection label="Motion" />
        <TweakToggle label="Background & celebration motion" value={motion} onChange={v => setTweak('motion', v)} />
