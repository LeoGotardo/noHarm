...
export function BadgeDetail({ onBack, badge, currentDays, justUnlocked }) {
  ...
  useEffect(() => {
    if (justUnlocked) {
      const t = setTimeout(() => setRipple(false), 1200);
  ...
  return (
    <Screen geo="badgeDetail" padTop={56} pulseKey={justUnlocked ? 1 : 0}>
            ...
            [0, 1].map((i) => (
              <span
                ...
                style={{
                  ...
                  animation: `nhCheckRipple 1.1s ${i * 0.25}s ease-out both`,
                }}
              />
            ))}
      ...
      {earned && (
        <div
          style={{ padding: "0 24px 16px", position: "relative", zIndex: 1 }}
        >
          <Btn kind="soft" full icon="share">
