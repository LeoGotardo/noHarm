import { Card, Icon, Btn, Avatar } from '@ui'
import { Screen, Header, StreakRing, hashHue } from '@components'
import { StatTile } from './StatTile.jsx'

export function Dashboard({ me, days, hasStreak, checkedIn, milestone, startLabel, personalRecord, totalStreaks,
  nextBadgeName, onCheckIn, onRelapse, onOpenHistory, onProfile, onStartStreak, pulseKey }) {
  ...
  return (
    <Screen geo="home" pulseKey={pulseKey} padTop={64}>
      ...
      {!hasStreak ? (
        <div style={{ padding: '32px 22px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--primary-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px' }}>
              <Icon name="flame" size={34} color="var(--primary)" sw={1.4} />
            </div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 'var(--display-weight)', fontSize: 24, color: 'var(--ink)', lineHeight: 1.2 }}>
              Begin your journey
            </div>
            <div style={{ fontSize: 14.5, color: 'var(--ink-2)', marginTop: 10, lineHeight: 1.55, padding: '0 12px' }}>
              Every recovery starts with a single day. When did yours begin?
            </div>
          </div>
          <div style={{ width: '100%' }}>
            <Btn kind="primary" size="lg" full icon="flame" onClick={onStartStreak}>Start my streak</Btn>
          </div>
        </div>
      ) : (
        <>
