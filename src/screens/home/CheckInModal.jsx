...
export function CheckInModal({
  ...
  const dayList = useMemo(() => {
    ...
    for (let i = 1; i <= missedDays; i++)
      list.push(addDays(lastCheckinDate, i));
    return list;
  }, [lastCheckinDate, missedDays, today]);

  const relapses = dayList
  ...
  const toggle = (date) =>
    setDays((prev) => ({
      ...prev,
      [date]: {
        checked: !prev[date]?.checked,
        time: prev[date]?.time || "12:00",
      },
    }));
      ...
      <div style={{ padding: "0 4px 8px" }}>
          ...
          <>
            ...
            >
              ...
              <div>
                ...
                >
                  Select the days you relapsed
                </div>
                <div style={{ fontSize: 13, color: "var(--ink-3)" }}>
                  Tap a day to mark it as a setback
                </div>
              </div>
            </div>
            ...
            <Btn kind="primary" full size="lg" onClick={() => setStep(3)}>
              Continue →
            </Btn>
          </>
        )}
        ...
        {step === 3 && (
          <>
            ...
            <div style={{ marginBottom: 20 }}>
              {relapses.length === 0 ? (
                <SummaryLine
                  icon="check"
                  color="var(--primary)"
                  text={`${daysLabel} clean — amazing work`}
                />
              ) : (
                (() => {
            ...
            >
              Relapses are part of recovery. Each setback is followed by a new
              beginning.
            </div>
...
function buildSegments(relapses, lastCheckin, today, dayList) {
  ...
  for (const r of relapses) {
    ...
    if (cleanDays > 0) {
      segments.push({
        type: "clean",
        label: `${cleanDays} clean day${cleanDays > 1 ? "s" : ""}`,
      });
    }
    const timeLabel = r.time ? ` at ${fmtTime(r.time)}` : "";
    segments.push({
      type: "relapse",
      label: `Setback on ${fmtShortDay(r.date)}${timeLabel}`,
    });
    prev = r.date;
  }
  ...
  if (finalDays > 0) {
    segments.push({
      type: "clean",
      label: `${finalDays} clean day${finalDays > 1 ? "s" : ""} since last setback`,
    });
  }
